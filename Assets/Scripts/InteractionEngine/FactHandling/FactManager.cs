...
public class FactManager : MonoBehaviour
{
    ...
    {
        return GlobalStatic.stage.factState[GlobalStatic.stage.factState.Add(fact, out exists, samestep)];
    }

    public PointFact AddPointFact(RaycastHit hit, bool samestep = false)
    {
        return (PointFact) AddFactIfNotFound(new PointFact(hit.point, hit.normal, GlobalStatic.stage.factState), out _, samestep);
    }

    public PointFact AddPointFact(Vector3 point, Vector3 normal, bool samestep = false)
    {
        return (PointFact) AddFactIfNotFound(new PointFact(point, normal, GlobalStatic.stage.factState), out _, samestep);
    }

    public OnLineFact AddOnLineFact(string pid, string lid, bool samestep = false)
    {
        return (OnLineFact)AddFactIfNotFound(new OnLineFact(pid, lid, GlobalStatic.stage.factState), out _, samestep);
    }

    public LineFact AddLineFact(string pid1, string pid2, bool samestep = false)
    {
        return (LineFact)AddFactIfNotFound(new LineFact(pid1, pid2, GlobalStatic.stage.factState), out _, samestep);
    }

    public RayFact AddRayFact(string pid1, string pid2, bool samestep = false)
    {
        RayFact rayFact = (RayFact)AddFactIfNotFound(new RayFact(pid1, pid2, GlobalStatic.stage.factState), out bool exists, samestep);
        if (exists)
            return rayFact;

        //Add all PointFacts on Ray as OnLineFacts
        PointFact rayP1 = (PointFact)GlobalStatic.stage.factState[rayFact.Pid1];
        PointFact rayP2 = (PointFact)GlobalStatic.stage.factState[rayFact.Pid2];
    ...
    {
        return (AngleFact)AddFactIfNotFound(new AngleFact(pid1, pid2, pid3, GlobalStatic.stage.factState), out _, samestep);
    }
